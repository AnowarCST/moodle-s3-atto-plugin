...
/**
 * Enable s3 plugin button in the files group on installation
 *
 * @return void
 */
function xmldb_atto_s3_install() {
    ...
    if (strpos($toolbar, 'files')) {
        ...
        foreach ($groups as $i => $group) {
            ...
            if (trim($parts[0]) == 'files') {
                $groups[$i] = 'files = s3, ' . trim($parts[1]);
                $found = true;
            }
        }
        ...
        foreach ($groups as $i => $group) {
            ...
            if (trim($parts[0]) == 'insert') {
                $groups[$i] = 'insert = s3, ' . trim($parts[1]);
                $found = true;
            }
        }
        ...
        foreach ($groups as $i => $group) {
            ...
            if (trim($parts[0]) == 'other') {
                $groups[$i] = 'other = s3, ' . trim($parts[1]);
                $found = true;
            }
        }
    }
