 ...
 * @return void
 */
function atto_s3_strings_for_js() {
    global $PAGE;
    ...
    $PAGE->requires->strings_for_js($strings, 'atto_s3');
}
 ...
 * @return array
 */
function atto_s3_params_for_js($elementid, $options, $fpoptions) {
    global $USER;

    $s3_account = get_config('atto_s3', 'account');
    $s3_player = get_config('atto_s3', 'player');
    ...
    $params = array('disabled' => $disabled, 'area' => array(), 'usercontext' => null,
        's3_account' => $s3_account, 's3_player' =>$s3_player);
